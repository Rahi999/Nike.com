...
import { womenClothingFailureFunction, womenClothingLoadingFunction, womenClothingSuccessFunction } from '../Redux/AppReducer/action';
...
const WomenShoes = () => {
  ...
  useEffect(() => {
     ...
     axios.get(`https://intermediate-little-dibble.glitch.me/Women_shoes?_limit=${limit}&_page=${page}`,queryParams)
  ...
  useEffect(() => {
    dispatch(womenClothingLoadingFunction());
    axios.get("https://intermediate-little-dibble.glitch.me/Women_shoes")
    .then((res)=> {
      setAllData(res.data)
  ...
  const {womenClothesLoading,womenClothesError,womenClothes} = useSelector((state) => {
    return {
      womenClothesLoading : state.AppReducer.womenClothesLoading,
      womenClothesError :   state.AppReducer.womenClothesError,
      womenClothes :        state.AppReducer.womenClothes
    }
  })
  
  return womenClothesLoading ? (<div id="menClothingContainer"><img width="30%" style={{marginLeft:"30%"}} src="https://i.pinimg.com/originals/b4/4e/22/b44e229598a8bdb7f2f432f246fb0813.gif"  alt="Loading Logo"/></div>)
    : womenClothesError ? (<div id="menClothingContainer"><img width="35%" style={{marginLeft:"30%"}} src="https://gifimage.net/wp-content/uploads/2018/11/something-went-wrong-gif-2.gif" alt="Error Logo" /></div>)
    :(
    <Box id="menClothingContainer">
      <Box style={{display:"flex",justifyContent : "space-between",padding:"10px"}}>
      <Typography id="h3">Women's Shoes({allData ? allData.length : 0})</Typography>
      ...
      <Box id="menClotheContainer" >
        ...
        <Box id="container2" >
          {womenClothes && womenClothes.map((items) => (
           <Link key={items.id} to={`/womenShoes/${items.id}`} style={{textDecoration:"none"}}>
             <Box id="mappedBox" >
